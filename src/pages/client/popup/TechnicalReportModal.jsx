...
const TechnicalReportModal = ({ tender, onClose }) => {
  ...
  const reportData = {
    ...
    approved: tender.bids.filter(b => b.status === 'technical-approved' || b.status === 'approved').length,
    disqualified: tender.bids.filter(b => b.status === 'disqualified').length,
    pending: tender.bids.filter(b => b.status === 'pending').length
  };
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6">
            <div className="text-center py-6">
              <h1 className="text-2xl font-bold text-gray-900 mb-2">Technical Evaluation Report</h1>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
                ...
                <p className="text-3xl font-bold text-blue-600">{reportData.totalBids}</p>
              </div>
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center">
                ...
                <p className="text-3xl font-bold text-red-600">{reportData.disqualified}</p>
              </div>
            </div>
                  ...
                  {tender.bids.map((bid, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-4 py-4">
                        ...
                        <div className="text-sm text-gray-500">{bid.contactPerson || 'N/A'}</div>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                        {bid.technicalScore > 0 ? `${bid.technicalScore}/100` : 'N/A'}
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ${
                          bid.status === 'technical-approved' || bid.status === 'approved'
