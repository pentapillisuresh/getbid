...
function PricingSection() {
  ...
  useEffect(() => {
    ...
    const fetchPlans = async () => {
      ...
      setError(null);
      try {
        const res = await api.get('/v1/plans', { queryParams: { page: 1, limit: 10 }, showToasts: false });
          ...
          { (loading ? fallbackPlans : plans).map((plan, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {plan.icon ? (
                    ...
                    React.isValidElement(plan.icon) ? (
                      ...
                      className: `w-10 h-10 ${plan.isPopular ? 'text-green-600' : 'text-purple-600'} transition-all duration-300`,
                      })
                      ...
                      <Target className={`w-10 h-10 ${isHovered ? 'text-purple-700' : 'text-purple-600'} transition-colors duration-300`} />
                    )
                  ) : (
                    <Target className={`w-10 h-10 ${isHovered ? 'text-purple-700' : 'text-purple-600'} transition-colors duration-300`} />
                  )}
                ...
                <p className="text-gray-500 text-sm mb-2">{plan.tendersLimit ? `${plan.tendersLimit} Tender Submissions` : plan.submissions}</p>
                <div className="text-3xl font-bold text-purple-700 mb-2">₹{typeof plan.price === 'number' ? plan.price.toLocaleString() : plan.price}</div>
                <p className={`text-sm font-medium mb-6 ${plan.color === "green" ? "text-green-600" : "text-purple-600"}`}>
                  ⏳ {plan.validDays ? `Valid for ${plan.validDays} days` : plan.duration}
                </p>
