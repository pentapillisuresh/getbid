...
const DocumentRepository = () => {
  ...
  const [documents, setDocuments] = useState([
    {
      ...
      description: 'Official company registration certificate from Registrar of Companies',
  ...
  const categories = [
    ...
    { value: 'certificates', label: 'Legal Documents', count: documents.filter(d => d.type === 'certificates').length },
    { value: 'proposals', label: 'Proposals', count: documents.filter(d => d.type === 'proposals').length },
    { value: 'contracts', label: 'Contracts', count: documents.filter(d => d.type === 'contracts').length },
    { value: 'financial', label: 'Financial Documents', count: documents.filter(d => d.type === 'financial').length },
    { value: 'technical', label: 'Technical Documents', count: documents.filter(d => d.type === 'technical').length }
  ];
              ...
              <div className="p-6">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${
                        ...
                        {doc.status === 'expiring' ? 'Verified' : 
                         doc.status === 'expired' ? 'Expired' : 'Verified'}
                      </span>
                    </div>
                  </div>
                ...
                <div className="space-y-2 text-xs text-secondary-500 mb-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    {doc.expiryDate && (
                      <span className="font-medium text-red-600">Expires: {doc.expiryDate}</span>
                    )}
                  </div>
                </div>
